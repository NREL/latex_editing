...
This change in page and float numbering is implemented using the \begin{verbatim}\frontmatter\end{verbatim}, \begin{verbatim}\mainmatter\end{verbatim}, and \begin{verbatim}\backmatter\end{verbatim} commands at the start of these sections of the document:
...
\label{Sec:Bib}
Use \texttt{bibtex} to organize references and store them in a single file (e.g. \begin{verbatim}/Documents/bibliography/bibliography.bib\end{verbatim}). The bibliography will then contain entries with `keys' for each source, like \texttt{Lamport\_1986\_a}.
...
\begin{itemize}
  \item \begin{verbatim}\cite{moriarty2020american}\end{verbatim} prints \cite{moriarty2020american}.
  \item \begin{verbatim}\citep{moriarty2020american}\end{verbatim} prints \citep{moriarty2020american}.
  \item \begin{verbatim}\citet{moriarty2020american}\end{verbatim} prints \citet{moriarty2020american}.
  \item A more complex case: \begin{verbatim}\parentext{\citet{moriarty2020american} and \citep{fernando2019perdigao}}\end{verbatim} prints \parentext{\citet{moriarty2020american} and \citep{fernando2019perdigao}}
\end{itemize}
...
To change the syntax highlighting use \begin{verbatim}\lstset{language=[dialect]language,columns=fullflexible,keepspaces=true}\end{verbatim} before each listing where the language changes. For more details see the \texttt{lstlisting} documentation.
