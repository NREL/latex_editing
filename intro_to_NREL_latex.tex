...
To cite URLs, use the 'misc' style. For example, the bibtex entry for \url{http://tex.stackexchange.com}\cite{texstackexchange} looks like this:
...
The \texttt{latex2rtf} program reads \LaTeX\ files and converts common \LaTeX\ commands into their RTF equivalent. It is effectively another \LaTeX\ interpreter that knows a limited subset of \LaTeX. See the documentation at \url{http://sourceforge.net/projects/latex2rtf/} for details.
...
\section{What to do when the conversion to rich text format fails}
